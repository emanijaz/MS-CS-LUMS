...
server <- function(input, output, session) {
  ...
  observe({
    ...
    if(FALSE)
    {
      ...
      cols <- c("hourly_load","Lag48", "Lag168", "Lahore_apparentTemperature")
      ...
      scaled[, c(1:4)] = lapply(scaled[ ,c(1:4)], as.numeric)
      
      
      
      # fit neural network
      NN_winter = neuralnet(hourly_load~., data = scaled[scaled$Season == "Winter",-5], hidden = c(6, 12, 6),
                            linear.output = T, stepmax = 1e7, lifesign = 'full')
      
      # fit neural network
      NN_summer = neuralnet(hourly_load~., data = scaled[scaled$Season == "Summer",-5], hidden = c(6, 12, 6),
                            linear.output = T, stepmax = 1e7, lifesign = 'full')
      
      # fit neural network
      NN_autumn = neuralnet(hourly_load~., data = scaled[scaled$Season == "Autumn",-5], hidden = c(6, 12, 6),
                            linear.output = T, stepmax = 1e7, lifesign = 'full')
      
      # fit neural network
      NN_spring = neuralnet(hourly_load~., data = scaled[scaled$Season == "Spring",-5], hidden = c(6, 12, 6),
                            linear.output = T, stepmax = 1e7, lifesign = 'full')
      ...
      if(Sys.time() > as.POSIXct(paste(Sys.Date(), "10:00:00 PKT"))){
        ...
        if(need_new_forecast){
            ...
            # if(FALSE)
          {
            ...
            scaled_NPCC_Prediction[, c(1:4)] = lapply(scaled_NPCC_Prediction[ ,c(1:4)], as.numeric)
      ...
      if(Sys.time() > as.POSIXct(paste(Sys.Date(), "10:00:00 PKT"))){
        ...
        if(is.null(week_forecast)){
          ...
          for(i in 2:7){
            ...
            forecast_df$Lag48 = historic$hourly_load[(nrow(historic)-23):nrow(historic)]
            ...
            ## Save and filter forecast data
            New_Forecast = forecast_df[,colnames(historic)]
